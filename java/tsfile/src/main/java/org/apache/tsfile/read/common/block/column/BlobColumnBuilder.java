...
public class BlobColumnBuilder implements ColumnBuilder {
  ...
  private static final long INSTANCE_SIZE =
      RamUsageEstimator.shallowSizeOfInstance(BlobColumnBuilder.class);

  private final ColumnBuilderStatus columnBuilderStatus;
  public static final BlobColumn NULL_VALUE_BLOCK =
      new BlobColumn(0, 1, new boolean[] {true}, new Binary[1]);
  ...
  public BlobColumnBuilder(ColumnBuilderStatus columnBuilderStatus, int expectedEntries) {
  ...
  public ColumnBuilder writeBinary(Binary value) {
    ...
    if (columnBuilderStatus != null) {
      columnBuilderStatus.addBytes(
          BlobColumn.SHALLOW_SIZE_IN_BYTES_PER_POSITION
  ...
  public ColumnBuilder writeObject(Object value) {
    ...
    throw new UnSupportedDataTypeException("BlobColumn only support Binary data type");
  }
  ...
  public ColumnBuilder appendNull() {
    ...
    positionCount++;
    if (columnBuilderStatus != null) {
      columnBuilderStatus.addBytes(BlobColumn.SHALLOW_SIZE_IN_BYTES_PER_POSITION);
    }
    return this;
  }
  ...
  public ColumnBuilder newColumnBuilderLike(ColumnBuilderStatus columnBuilderStatus) {
    return new BlobColumnBuilder(columnBuilderStatus, calculateBlockResetSize(positionCount));
  }
