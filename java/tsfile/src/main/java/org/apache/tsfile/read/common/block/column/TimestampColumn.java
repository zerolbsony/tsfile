...
public class TimestampColumn implements Column {
  ...
  private static final int INSTANCE_SIZE =
      (int) RamUsageEstimator.shallowSizeOfInstance(TimestampColumn.class);
  ...
  public TimestampColumn(int initialCapacity) {
    this(0, 0, null, new long[initialCapacity]);
  }

  public TimestampColumn(int positionCount, Optional<boolean[]> valueIsNull, long[] values) {
    this(0, positionCount, valueIsNull.orElse(null), values);
  }

  TimestampColumn(int arrayOffset, int positionCount, boolean[] valueIsNull, long[] values) {
    if (arrayOffset < 0) {
  ...
  public Column getRegion(int positionOffset, int length) {
    ...
    return new TimestampColumn(positionOffset + arrayOffset, length, valueIsNull, values);
  }
  ...
  public Column getRegionCopy(int positionOffset, int length) {
    ...
    return new TimestampColumn(0, length, valueIsNullCopy, valuesCopy);
  }
  ...
  public Column copyPositions(int[] positions, int offset, int length) {
    ...
    return new TimestampColumn(0, length, newValueIsNull, newValues);
  }
